...
class ChatAdapter1(mcontext: Context,
                   mchatlist:List<Chat>
):RecyclerView.Adapter<ChatAdapter1.ViewHolder?>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        Glide.with(mcontext).load(chat.profile_url).placeholder(R.drawable.my_profile_image1)
            .centerCrop().into(
        ...
        holder.chat_profile_image!!.setOnClickListener {
            ...
            dialog.setIcon(R.drawable.lost_and_foundreal)
            dialog.setTitle("What do you want?")
            dialog.setItems(decisions, DialogInterface.OnClickListener { dialog, which ->
                if (which == 0) {
        ...
        if (chat.message == "image sent9430828829" && chat.url!=""){
            ...
            if (chat.sender_id == firebaseuser.uid) {
                ...
                Glide.with(mcontext).load(chat.url)
                    .placeholder(R.drawable.baseline_sensor_occupied_24)
                    .into(holder.chat_image_right!!)
    ...
    private fun delete_message(position: Int, holder: ChatAdapter1.ViewHolder) {
        val mdataref1 = FirebaseDatabase.getInstance().getReference("Found_Chats")
